...
Summary(pl.UTF-8):	Sterownik dla Linuksa do kart bezprzewodowych USB opartych na uk≈Çadzie zd1211
Name:		zd1211
Version:	%{_zd1211_ver}
...
Linux Kernel Treiber f√ºr WLAN Netzwerkkarten zd1211.

%package -n kernel%{_alt_kernel}-net-%{name}
Summary:	Linux kernel module for WLAN cards based on zd1211 
Summary(de.UTF-8):	Linux Kernel Modul f√ºr WLAN Netzwerkkarten zd1211
Summary(pl.UTF-8):	Modu≈~B jƒ~Edra Linuksa dla kart WLAN na zd1211
Release:	%{_rel}@%{_kernel_ver_str}
Group:		Base/Kernel
...
Dieses Paket enth√§lt Linux Kernel Treiber f√ºr WLAN Netzwerkkarten zd1211.

%description -n kernel%{_alt_kernel}-net-%{name} -l pl.UTF-8
Ten pakiet zawiera sterowniki jƒ~Edra Linuksa dla kart WLAN na zd1211.

%package -n kernel%{_alt_kernel}-smp-net-%{name}
Summary:	Linux SMP kernel module for the WLAN cards based on zd1211.
Summary(de.UTF-8):	Linux SMP Kernel Modul f√ºr WLAN Netzwerkkarten zd1211
Summary(pl.UTF-8):	Modu≈~B jƒ~Edra Linuksa SMP dla kart WLAN na zd1211
Release:	%{_rel}@%{_kernel_ver_str}
Group:		Base/Kernel
...
Dieses Paket enth√§lt Linux SMP Kernel Treiber f√ºr WLAN Netzwerkkarten zd1211.

%description -n kernel%{_alt_kernel}-smp-net-%{name} -l pl.UTF-8
Ten pakiet zawiera sterowniki jƒ~Edra Linuksa SMP dla kart WLAN opartych na uk≈~Badzie zd1211.
